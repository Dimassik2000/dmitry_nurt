...
def compute_metrics(eval_pred):
    ...
    # Приведение размеров предсказаний и меток
    logits = [logit[:labels.shape[1], :labels.shape[2]] for logit in logits]  # обрезаем до размеров меток

    # Проверка размеров перед вычислением метрики
    if logits[0].shape != labels.shape[1:]:
        raise ValueError(f"Размеры не совпадают: {logits[0].shape} и {labels.shape[1:]}")
