...
def compute_metrics(eval_pred):
    ...
    logits_resized = [
        F.interpolate(torch.tensor(logit).unsqueeze(0).unsqueeze(0).float(), size=labels.shape[1:], mode="bilinear", align_corners=False).squeeze().long().numpy().tolist()
        for logit in logits_resized
    ]
