...
def compute_metrics(eval_pred):
    ...
    logits = np.argmax(logits, axis=1)  # получаем метки для каждого пикселя

    # Приведение размеров предсказаний к размерам меток
    logits_resized = [logit[:labels.shape[1], :labels.shape[2]] for logit in logits]

    # Приведение размера предсказаний к размеру меток
    logits_resized = [F.interpolate(torch.tensor(logit).unsqueeze(0).unsqueeze(0).float(), size=labels.shape[1:], mode="bilinear", align_corners=False).squeeze().long().numpy() for logit in logits_resized]

    # Проверка размеров перед вычислением метрики
    if logits_resized[0].shape != labels.shape[1:]:
        raise ValueError(f"Размеры не совпадают: {logits_resized[0].shape} и {labels.shape[1:]}")
...
val_predictions = []
with torch.no_grad():
    for val_image, val_annotation in zip(val_images, val_annotations):
        val_image = Image.fromarray(val_image).resize((512, 512))
        ...
        # Приведение размера предсказаний к истинным меткам
        predicted_mask = predicted_mask[:val_annotation.shape[0], :val_annotation.shape[1]]
