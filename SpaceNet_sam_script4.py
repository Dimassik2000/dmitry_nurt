...
def check_labels(labels, num_labels):
    if torch.any(labels < 0) or torch.any(labels >= num_labels):
        print("Некорректные метки:", labels)
...
def compute_metrics(eval_pred):
    ...
    labels = labels.squeeze(1)  # Убираем лишние измерения

    # Обеспечение совпадения размеров предсказаний и истинных меток
    logits = [logit[:labels.shape[1], :labels.shape[2]] for logit in logits]
...
with torch.no_grad():
    for val_image, val_annotation in zip(val_images, val_annotations):
        ...
        # Проверка аннотации на допустимость меток
        val_annotation = np.clip(val_annotation, 0, num_labels - 1)  # Обрезаем значения меток
